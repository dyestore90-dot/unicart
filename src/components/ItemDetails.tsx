import { ArrowLeft, Plus, Minus, Store, Lock } from 'lucide-react'; // Added Lock icon
      ...
      <div className="relative w-full max-w-md h-full bg-black text-white shadow-2xl flex flex-col animate-in slide-in-from-bottom duration-200">
        ...
        <div className="flex-1 overflow-y-auto scrollbar-hide">
          <div className="relative h-72 w-full bg-[#252525]">
            ...
            <button onClick={onClose} className="absolute top-4 left-4 p-2 bg-black/50 backdrop-blur-md rounded-full text-white hover:bg-black/70 transition-colors z-10">
          ...
          <div className="p-5 pb-32">
            <div className="mb-8">
              ...
              <div className="bg-[#1a1a1a] p-4 rounded-xl border border-gray-800">
                ...
                <p className="text-gray-200 text-sm leading-relaxed">{item.description || "No description available."}</p>
              </div>
            </div>
        ...
        <div className="bg-[#1a1a1a] border-t border-gray-800 p-5 pb-8 z-20">
          {isClosed ? (
            <button disabled className="w-full bg-[#252525] text-gray-500 font-bold py-3.5 rounded-xl cursor-not-allowed flex items-center justify-center gap-2">
              <Lock className="w-5 h-5" /> Restaurant Currently Closed
            </button>
          ) : quantity === 0 ? (
            <button onClick={() => addToCart(item)} className="w-full bg-[#c4ff00] text-black font-bold py-3.5 rounded-xl hover:bg-[#b3e600] transition-transform active:scale-95 text-lg shadow-lg shadow-[#c4ff00]/20">
            ...
            <div className="flex items-center justify-between gap-4">
              ...
              <div className="flex items-center bg-[#c4ff00] text-black rounded-xl px-2 py-1.5 gap-4 shadow-lg shadow-[#c4ff00]/10">
                <button onClick={() => updateQuantity(item.id, quantity - 1)} className="w-10 h-10 flex items-center justify-center bg-black/10 rounded-lg hover:bg-black/20"><Minus className="w-5 h-5" /></button>
                <span className="font-bold text-xl min-w-[24px] text-center">{quantity}</span>
                <button onClick={() => updateQuantity(item.id, quantity + 1)} className="w-10 h-10 flex items-center justify-center bg-black/10 rounded-lg hover:bg-black/20"><Plus className="w-5 h-5" /></button>
              </div>
            </div>
          )}
        </div>
